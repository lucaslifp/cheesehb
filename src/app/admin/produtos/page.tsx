...
export default function AdminProdutosPage() {
  ...
  const [filtroStatus, setFiltroStatus] = useState<
    "todos" | "ativo" | "inativo"
  >("todos");
  const [filtroCardapio, setFiltroCardapio] = useState<"todos" | "sim" | "nao">(
    "todos"
  );
  ...
  const [produtoToDelete, setProdutoToDelete] = useState<ProdutoAdmin | null>(
    null
  );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/produtos/${produtoToDelete.id}`,
        { method: "DELETE" }
      );
      if (!response.ok) {
      ...
      toast({
        ...
        description: `"${produtoToDelete.nome}" foi removido.`,
      });
    } catch (e: any) {
      toast({
  ...
  const categoriasUnicas = useMemo(() => {
    const nomesUnicos = Array.from(
      new Set(produtos.map((p) => p.nome_categoria).filter(Boolean))
    );
  ...
  const filteredProdutos = useMemo(() => {
    return produtos.filter((produto) => {
      ...
      const matchCardapio =
        ...
        (filtroCardapio === "sim" && produto.mostrar_no_cardapio) ||
        (filtroCardapio === "nao" && !produto.mostrar_no_cardapio);
      const matchAdicional =
        filtroAdicional === "todos" ||
        (filtroAdicional === "sim" && produto.usar_como_adicional) ||
        (filtroAdicional === "nao" && !produto.usar_como_adicional);
      const matchCategoria =
        filtroCategoria === "todos" ||
        produto.nome_categoria === filtroCategoria;
      return (
        matchNome &&
        matchStatus &&
        matchCardapio &&
    ...
    <div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Visualize, edite ou adicione novos produtos (bebidas, hambúrgueres,
            etc.).
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <TableRow>
                    ...
                    <TableHead className="w-[120px]">
                      <div className="flex flex-col gap-1">
                        Cardápio
                        <Select
                          value={filtroCardapio}
                          onValueChange={(value) =>
                            setFiltroCardapio(value as any)
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                            <SelectValue />
                    ...
                    <TableHead className="w-[120px]">
                      <div className="flex flex-col gap-1">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setFiltroAdicional(value as any)
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                            <SelectValue />
                    ...
                    <TableHead className="min-w-[150px]">
                      <div className="flex flex-col gap-1">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setFiltroCategoria(value as any)
                          }
                        >
                          <SelectTrigger className="h-7 text-xs">
                            <SelectValue />
                    ...
                    <TableRow key={produto.id}>
                      ...
                      <TableCell>
                        ...
                        {produto.preco_base != null
                          ? produto.preco_base.toFixed(2).replace(".", ",")
                          : "—"}
                      </TableCell>
                      ...
                      <TableCell className="text-center">
                        <Badge
                          variant={
                            produto.mostrar_no_cardapio ? "default" : "outline"
                          }
                          className="cursor-default"
                        >
                          {produto.mostrar_no_cardapio ? (
                            <Eye className="mr-1 h-3 w-3" />
                          ) : (
                            <EyeOff className="mr-1 h-3 w-3" />
                          )}
                      ...
                      <TableCell className="text-center">
                        <Badge
                          variant={
                            produto.usar_como_adicional ? "default" : "outline"
                          }
                          className="cursor-default"
                        >
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setProdutoToDelete(null)}
              disabled={isSubmitting}
            >
              Cancelar
            </AlertDialogCancel>
